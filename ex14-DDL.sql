...
데이터 유형
    ...
    BFILE 파일위치와 매타데이터 저장(최대 4GB)
*/

-- departments 테이블에서 dept에 데이터 복사하기
INSERT INTO dept 
...
-- DROP 문 : 객체를 삭제할 때 사용되는 SQL 명령문입니다.
DROP TABLE simple;

/*
제약조건(Constraint)
    테이블의 해당컬럼에 원하지 않는 데이터를 입력/수정/삭제 되는 것을 방지하기
    위해 테이블 생성 또는 변경 시 설정하는 조건입니다.(저장된 데이터의 신뢰성 높이기 위함) 
    
    NOT NULL
        NULL로 입력이 되어서는 안되는 컬럼에 부여하는
        조건으로 컬럼 레벨에서만 부여할 수 있는 제약조건입니다.
    UNIQUE KEY(유일키)
        저장된 값이 중복되지 않고 오직 유일하게 유지되어야 할때
        사용하는 제약조건입니다.
    PRIMARY KEY(대표키, 기본키)
